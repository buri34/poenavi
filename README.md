...
### 🧭 ぽえなび / ぽえとれ 起動モード
- 起動時に、攻略ガイド中心の**ぽえなび**と価格検索中心の**ぽえとれ**を選択できます
- 選んだモードを記憶して次回から直接起動することも、起動のたびに選択することもできます
- 設定画面から次回起動モードを変更でき、必要に応じて確認後に再起動して切り替えます
- 共通メモ・Cheat sheets・アプリ情報・手動アップデート確認は両モードから利用できます
- ぽえなびは黄緑、ぽえとれは紫を基調としたテーマで表示します

### 📖 攻略ガイド
- PoE1 / PoE2 のAct進行に合わせて、エリアごとの**目標・レイアウト情報・Tips**を自動表示
...
### 🧭 みになび（PoE1のみ）
- Act進行中に必要な行動だけを、ゲーム画面へ重ねやすい小さな別ウィンドウで表示します
- エリア移動を検知すると、目的地・クエスト・ボス・ウェイポイントなどの案内が自動で切り替わります
- 進行方向の矢印と、現在レベルに応じた経験値効率の目安も表示します
- ウィンドウは好きな位置・サイズへ移動でき、鍵ボタンで固定できます
- ロック中はクリックが背後のゲーム画面へ通るため、表示したままプレイできます
- フォントサイズ、ウィンドウ透過率、文字透過率、常に最前面、自動フェードを設定画面から調整できます

> [!NOTE]
> みになびは現在PoE1専用です。筋が良さそうだったら、PoE2にも展開予定です。
...
- **Ready自動スタート（PoE1）** — タイマーをリセットした状態で `Ready` を押すと待機状態になり、新キャラクターが「黄昏の岸辺（The Twilight Strand）」へ入場したログを検知して自動でタイマーを開始します。
  - Ready中はClient.txtの監視間隔を100msへ短縮し、開始後は通常の監視間隔へ戻ります
  - Readyをもう一度押すと待機を解除できます。Ready状態はアプリ再起動後へ持ち越しません
  - タイマー記録が残っている場合は、内容を確認してResetしてからReadyにしてください
  - 手動のStart操作も従来どおり利用できます
  - PoE側でローカルチャットログを無効にしていると入場ログを検知できないため、オプションのチャット設定で「ローカル」を有効にしてください
...
### 🗒️ エリア別ユーザーメモ
- ガイド欄の「📝 エリアメモ」から、エリアごとに自由記述メモを1件保存できます
- 既存メモと同じ7色の文字色に対応
- 内容があるエリアだけ公式ガイドの最上部へ表示し、空欄時は表示領域を作りません
- 公式ガイドとは別のユーザーデータとして保存するため、アップデート後も保持されます
...
- `Client.txt`を読み取り、エリア移動・レベルアップ・Act進行を検知
- 攻略ガイド、マップ画像、XP目安、RTAタイマーをローカル表示
- 設定されたグローバルホットキーを監視
- ユーザー操作を起点に、チャットコマンドや検索文字列の貼り付け・送信などのキー入力を実行
- ユーザーがログアウトホットキーを押した時、PoEクライアントのTCP接続を切断
- 設定、メモ、ガイド編集内容、タイマー記録などをPC内へ保存

#### ぽえとれが行うこと

- Alt+D操作時にPoEへ`Ctrl+C`を送り、クリップボードからアイテム情報を取得
- アイテム情報を解析し、公式Trade用の検索条件を作成
- 公式Trade APIから出品情報を取得
- poe.ninjaから通貨・アイテムの参考価格を取得
- アイテム画像をPoE公式CDNなどから取得
- ユーザー操作に応じて公式Tradeやpoe.ninjaのページをブラウザで開く
- 設定されたグローバルホットキーを監視し、チャットコマンド送信などの入力補助を実行
- 設定や価格キャッシュをPC内へ保存

ぽえとれモードでは、基本的に`Client.txt`を使用しません。
...
- ネットワークパケットの傍受・改ざん
- 画面認識などによる自律的な戦闘・移動・アイテム操作
- ユーザー操作なしで継続的にゲームをプレイするBot動作
- `Client.txt`の内容や個人メモの外部送信
- PoEアカウントの認証情報やセッション情報の収集

PoEのウィンドウを安全に識別するため、Windows APIでウィンドウ位置とプロセス名を確認しますが、ゲームメモリの内容は読み書きしません。

#### 外部通信先と送信内容

| 通信先 | 用途・送信内容 |
|------|------|
| GitHub Releases | アップデート確認。ログ内容やプレイデータは送信しません |
| Path of Exile公式Trade API | ぽえとれの価格検索。アイテム名、MOD、数値などの検索条件を送信します |
| poe.ninja | 通貨・アイテムの参考価格取得 |
| Path of Exile公式CDNなど | アイテム画像などの取得 |
| PoELab | ユーザー操作時に当日のラビリンス情報をブラウザで開きます |

- **ソースコード完全公開** — 処理内容はこのリポジトリで確認できます
...
通知を閉じた後は、設定画面の **アプリ情報 → アップデートを確認** から再確認できます。ぽえなびでは本体の右クリックメニューからも確認できます。ソースから実行している場合は、自動置換の代わりにReleaseページを案内します。
...
2. **ぽえなび**または**ぽえとれ**を選択
   - 「次回からこのモードで直接起動」をONにすると、次回から選択画面を省略できます
3. ぽえなびを選んだ場合は、PoE1 / PoE2 のどちらで起動するかを選択
4. ⚙ボタン（または右クリック →「設定」）→ **基本設定** タブで、選択中モードに対応する `Client.txt` のパスを設定
...
5. ゲーム内チャットの表示設定で **「ローカル」** を表示ONにする
...
3. `Alt+D`を押す
4. ぽえとれ画面で検索条件と出品結果を確認する

検索後もPoEのフォーカスを維持するため、別のアイテムへカーソルを移して続けて`Alt+D`を押せます。検索条件を編集する場合は、ぽえとれ画面内をクリックしてください。画面外をクリックすると閉じます。

ホットキーは、設定画面の**ホットキー → ぽえとれ検索**から変更・解除できます。

ぽえとれモードで最小化すると、アプリはWindowsの通知領域へ格納されます。
トレイアイコンから、ぽえとれの表示、設定画面の表示、アプリの終了ができます。

### PoE1: Map Modチェック

Map系アイテムへカーソルを合わせて`Alt+F`を押すと、Trade APIへ通信せず、
コピーしたModを危険・警告・有利のラベルで色分け表示します。ホットキーは
設定画面から変更できます。
...
ユニークMapと通常アイテムはMap Modチェックの対象外です。

### PoE1: OBS配信用ウィンドウ

設定画面の **OBS配信 → 検索結果ウィンドウをOBS配信用にする** をONにすると、
ぽえとれ本体とは別に、OBSのウィンドウキャプチャ用ウィンドウを常設します。

- 待機中はドラッグ可能なタイトルバーだけを表示
- 価格検索時は、同じウィンドウの下へ検索結果を展開
- Esc、右上の×、画面外クリックでは終了せずタイトルバーへ折りたたみ
- OBSではウィンドウ名 `ぽえとれ - 検索結果ウィンドウ` を選択

検索処理、公式Trade API、poe.ninja、キャッシュは通常モードと共通で、
OBS用に同じ検索を二重実行することはありません。

### 任意チャットコマンド

設定画面の**任意コマンド設定**タブでは、PoEチャットへ送るコマンドと
グローバルホットキーの組み合わせを自由に登録できます。

- ぽえなび／ぽえとれの両モードで利用可能
- 名前、ホットキー、`/`で始まるチャットコマンド、有効／無効を設定可能
- ホットキー欄をクリックし、実際に押したキーまたは複合キーを登録
- 既存機能と重複するホットキーは保存不可

例: `Ctrl+H`に`/hideout`、`Ctrl+K`に`/kingsmarch`を登録できます。

> [!NOTE]
> - ぽえとれはPoE1専用です。
> - 無料の非公式ツールであり、Grinding Gear Gamesによる提携・承認を受けたものではありません。
> - Valdo Mapの報酬条件検索は現在非対応です。画面上に案内を表示し、対応していない条件を除いて検索します。

### PoE1: みになびの使い方

1. PoE1モードで起動し、メイン画面のガイドを展開する
2. ガイド見出し右側の **「みになび OFF」** をクリックしてONにする
3. 表示されたみになびを、ゲーム画面の見やすい位置へ移動・リサイズする
4. 右上の鍵ボタンを押して位置を固定する

鍵が閉じている間はクリックが背後へ通ります。位置やサイズを変えたい場合は、鍵ボタンを押してロックを解除してください。

表示の見やすさは、⚙設定の **「ウィンドウ設定（みになび）」** から変更できます。ゲーム画面を隠したくない場合は、ウィンドウ・文字の透過率や、一定時間後に薄くなる自動フェードを調整してください。
...
- `area_notes_poe1.json` / `area_notes_poe2.json` — エリア別ユーザーメモ
...
- ぽえとれのデータ出典・第三者ライセンスは [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) を参照
