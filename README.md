...
- **⚠️ 管理者権限が必要です** — TCP接続の強制切断にはOSの管理者権限が必要です
  - **exe版**: 右クリック →「管理者として実行」で起動
  - **py版**: 同梱の `run_admin.bat` をダブルクリック（UAC確認後に管理者権限で起動）、または管理者として開いたコマンドプロンプトから `python main.py`
  - 管理者権限なしでもアプリは通常通り動作しますが、ログアウトマクロ使用時にエラーメッセージが表示されます
