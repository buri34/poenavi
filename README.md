...
- 検索文字列は手入力のほか、**正規表現の作成支援チェックボックス**でよく使う条件を組み合わせて作成できます
  - 共通条件、ビルド別条件、武器ベース条件に対応
  - 武器ベースを選ぶと、特定の武器種に限定した検索文字列になります
- PoE1モードでは、Act中の店売り装備検索に特化したプリセット画面になります
  - 3リンク / 2リンク装備の色組み合わせをチェックボックスで作成
  - 任意のソケット数・リンク数を指定できる「Other Links」に対応
  - PoE1用プリセットは `vendor_search_presets_poe1.json`、PoE2用プリセットは `vendor_search_presets_poe2.json` として別々に保存
...
PoE1モードでは、キャンペーンRTA・レベリング向けに以下の機能を強化しています。

- **PoBインポートによるジェム取得リスト**
  - Path of Building のエクスポートコードを読み込み、必要ジェムの取得タイミングを自動表示
  - クエスト報酬 / ベンダー購入 / Act6 Lilly 購入を区別して表示
  - 現在Actに合わせてジェムリストが自動追従
  - ジェム名クリックでPoE内検索欄へ貼り付け可能
...
- **PoE1向け店売り検索プリセット**
  - 3リンク装備など、Act中のベンダー検索向けプリセットをPoE1専用に管理
  - リンク色・ソケット色などのチェックボックスから検索文字列を生成できます
