...
**Client.txtログ監視による自動エリア検知**＋**攻略ガイド**＋**マップ画像表示**＋**RTAタイマー**をオーバーレイ表示。
...
- **訪問回数別ガイド** — 同じエリアの2回目訪問では別のガイドを表示可能
- ガイドデータはJSON形式で自由に編集可能

### 📍 自動エリア検知 & XPペナルティ判定
- **PoEのClient.txtログをリアルタイム監視** — エリア移動・レベルアップを自動検知
- 各エリアのモンスターレベルをもとに、XPペナルティを3段階で判定
  - 🟢 最適レベル — 余裕ラインの範囲内
  - 🟡 許容範囲 — ペナルティなしだが余裕を超えている
  - 🔴 ペナルティ — 経験値にペナルティ発生中
...
Settings → **エリア情報** タブで、Act 1〜10の各エリアを管理できます。

- **エリア名**: Client.txtに出力される名前と一致させてください
...
|----------|------|
| **自動（レベル）** | レベル39以上で自動切替 |
| **自動（エリア検知）** | Act 6-10固有エリアに入場で自動切替 |
| **手動ボタン** | UI上のボタンで手動切替 |
...
| ファイル | 説明 | Git管理 |
|----------|------|---------|
| `config.json` | 設定（ホットキー、ログパス、ゾーンレベル） | ✕（ユーザー固有） |
