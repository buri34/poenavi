...
- 検索ホットキーを押すと、有効なプリセットだけがメニュー表示され、選択した文字列を元のウィンドウへ `Ctrl+F` → 貼り付けできます
- プリセットは `vendor_search_presets.json` に保存されます（ユーザーごとの設定ファイルなのでGit管理対象外です）
...
### 🌐 英語クライアント対応（2026/5/5時点では、PoE1のみ対応）
