...
class LogWatcher(QObject):
    ...
    def _trace_twilight_strand(self, zone_name: str, line: str, source: str):
        """開発用: 黄昏の海岸を検知した瞬間をコンソールへ明示する。"""
        if os.environ.get("POENAVI_TWILIGHT_TRACE") != "1":
            return
        if zone_name not in self.TWILIGHT_STRAND_NAMES:
            return

        timestamp_match = self.LOG_TIMESTAMP_PATTERN.match(line.strip())
        log_timestamp = (
        ...
        print("", flush=True)
        print("=" * 72, flush=True)
        print(">>> [黄昏の海岸・検知テスト]", flush=True)
        print(f">>> 検知経路     : {source}", flush=True)
        print(f">>> Client.txt時刻: {log_timestamp}", flush=True)
        print(f">>> PoENavi検知時刻: {detected_at}", flush=True)
        print(f">>> エリア       : {zone_name}", flush=True)
        print(f">>> ログ原文     : {line.strip()}", flush=True)
        print("=" * 72, flush=True)
    ...
    def _parse_line(self, line: str):
        ...
        if m:
            ...
            self._trace_twilight_strand(zone_name, line, "入場ログ（日本語）")
            self.zone_entered.emit(zone_name)
            return
        ...
        if m:
            zone_name = m.group(1).strip()
            print(f"[LogWatcher] Zone detected: {zone_name} (pos={self._file_pos}, line={line.strip()[:80]})")
            self._trace_twilight_strand(zone_name, line, "入場ログ（英語）")
            self.zone_entered.emit(zone_name)
            return
        ...
        if m:
            ...
            self._trace_twilight_strand(zone_name, line, "Set Source")
            self.zone_entered.emit(zone_name)
            return
