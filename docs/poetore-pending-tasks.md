...
- [ ] PoE1／PoE2共通の状態切替条件「未鑑定」「Veiled／Veiledを含む」
      「Foil Unique／通常Unique」「スプリット品含む／非スプリット」
...
- [x] PoE2のSanctified品では「聖別化」をMirroredと同じ上部の専用検索チップとして表示する。
      `聖別品のみ`／`非聖別品のみ`／`聖別品含む`の3状態とし、聖別品では`聖別品のみ`を初期選択する。
      コラプトと異なり警告色は使わない。各状態で`sanctified=true`／`false`／条件なしを送り、
      `P2-WIN-035`の日英Sanctified Anvil MaulでWindows実機確認する
...
- [ ] ジクアニの光輝／罠のソウルコアは、Forbidden Ritesの実出品でそれぞれ
      「全てのヘラルドスキルジェムのレベル+1」「全てのハザードスキルジェムのレベル+1」を
      確認済み。公式Trade2 `data/stats?realm=poe2`に対応するRune Stat IDが追加されたら、
      仮挿入検索へ追加する。未確認IDは推測採用しない
