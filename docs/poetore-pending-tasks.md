...
更新設計は`docs/poetore-league-data-update-plan.md`に文書化済み。
固定入力からの各派生データ再生成と差分0確認は可能だが、候補抽出から
レビュー済み反映までを一つにまとめた運用ツールは未実装。

- [ ] 読み取り専用の総合監査コマンドを実装する
- [ ] レビュー済み候補の原子的反映と代表日本語Trade API確認を実装する
- [ ] 日英アイテム一覧の欠落・順序差を見やすい同期レポートにする
- [ ] pseudo派生データの追加・削除・変更レポートを更新フローへ組み込む
