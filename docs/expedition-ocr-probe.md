...
入力画像は、PoENavi本体と同じく「先頭カード左上から報酬パネル内側の右下」までを
切り抜いた範囲を使用します。タイトル・外枠・ゲーム背景は含めません。
...
相対的な明暗差で分離し、カード全体の文字高さを正規化した各報酬行と`result.json`が入り、全画像の結果は
...
数量はWindows OCRで実際に発生した`1x`→`lx`、`1 ,`、`1 =`などを補正します。
カード全体OCRではアイコン由来ノイズの後ろに数量マーカーが現れる場合もあるため、
マーカーより右側を報酬名候補として優先します。
PoENavi本体では通常のOCRで安全確定できなかった行だけ、局所適応二値化と大きめの
文字倍率、ルーンアイコンをマスクした画像を使って再OCRします。通常処理で既に
安定確定した行は再処理しません。
