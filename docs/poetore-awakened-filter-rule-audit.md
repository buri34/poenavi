...
CSVの`メンテナー判断欄`が`要判断`または`要確認`の行は、Awakenedへ機械的に揃える前に
...
9. **ExactのMagic rarity（準拠済み）**
   - AwakenedはAdorned対象外のMagic品について、Magic条件を初期OFF候補として持ち、
     初期APIにはrarityを送らない。
   - ぽえとれも初期APIからrarityを外し、手動選択時だけMagicを送る。

10. **Veiled非Uniqueのilvl（今回の再監査で発見）**
    - Awakenedは完成品プリセットでもilvlを強制的に初期ONにする。
    - ぽえとれは通常完成品と同じ初期OFFのため未対応。

11. **最大ライフ／マナ疑似Modのrequired条件（準拠済み）**
    - Awakenedは明示最大ライフModがある時だけ最大ライフ合計を生成し、
      明示最大マナModがある時だけ最大マナ合計を生成する。
    - ぽえとれも明示ライフ／マナをrequired条件とし、能力値だけでは生成しない。
    - 明示Modがあれば、筋力／知性による換算分も合計へ加える。

12. **強いクラフト価値を完成品判定より優先（ぽえとれ独自仕様）**
    - Awakenedは品質20%やCrafted Modがあれば、Fractured等があっても完成品だけにする。
    - ぽえとれは加工可能なFractured・Synthesised・Influence等なら、
      完成品／クラフトベースの両方を表示する。
...
pseudoのgroup/replaces定義そのものは、生成済みメタデータと回帰テストを正本とする。
